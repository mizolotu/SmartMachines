...
@register("mlp")
def mlp(num_layers=2, num_hidden=512, activation=tf.tanh, layer_norm=False):
    """
...
def cnn(nh=64):
    def network_fn(X):
        conv1 = tf.keras.layers.Conv1D(filters=nh, kernel_size=2, strides=2, activation='relu')(X)
